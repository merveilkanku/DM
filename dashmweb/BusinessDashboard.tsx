...
import { getBusinessInsights, getSmartSupportResponse, processVoiceCommand } from "./lib/gemini";
import {
  Plus,
  Trash2,
  Power,
...
export const BusinessDashboard: React.FC<Props> = ({
  ...
  const isExpired = restaurant?.subscriptionTier !== 'free' && (
  ...
  useEffect(() => {
    ...
    recognition.onresult = async (event: any) => {
      // Only process final results to avoid looping partial phrases
      ...
      if (result.isFinal) {
        ...
        toast.promise(processVoiceCommand(command, "business"), {
          loading: 'Analyse...',
          success: (data) => {
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === 'network') {
        toast.error("Problème réseau avec la reconnaissance vocale");
        setIsVoiceActive(false);
      }
    };

    recognition.onend = () => {
      // Graceful restart if still active and not an error state
    ...
    try {
      ...
      toast.success("Assistant vocal activé. Dites une commande.");
    } catch (e) {
  ...
  const handleVoiceAction = (data: any) => {
    if (!data || data.action === 'unknown') return;

    switch (data.action) {
      case 'update_status':
        const orderId = data.orderId || orders.find(o => o.status === 'pending' || o.status === 'confirmed')?.id;
        if (orderId && data.status) {
          updateOrderStatus(orderId, data.status as OrderStatus);
        }
        break;
      case 'navigation':
        if (data.view) navigateTo(data.view as DashboardView);
        break;
    }
  };
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          message: `Le restaurant ${restaurant.name} (ID: ${restaurant.id}) souhaite s'abonner au plan ${selectedPlan.name.toUpperCase()}. Prix: ${selectedPlan.price} USD. L'utilisateur attend une activation manuelle après avoir contacté l'administration.`,
          status: "open"
        });
  ...
  useEffect(() => {
    ...
    if (prevTierRef.current !== restaurant.subscriptionTier) {
       ...
       if (restaurant.subscriptionTier && restaurant.subscriptionTier !== 'free') {
         ...
         toast.info("ℹ️ Votre abonnement a été réinitialisé au forfait Gratuit.");
       }
    ...
    if (prevStatusRef.current !== restaurant.subscriptionStatus) {
       if (restaurant.subscriptionStatus === 'active' && prevStatusRef.current !== 'active') {
         toast.success("✅ Votre abonnement est désormais actif ! Profitez de toutes vos fonctionnalités.");
       } else if (restaurant.subscriptionStatus === 'expired') {
         toast.error("⚠️ Votre abonnement a expiré. Veuillez le renouveler pour garder vos avantages.");
       }
  ...
  const renderSubscriptionBanner = () => {
    if (restaurant.subscriptionTier === 'free') return null;
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-5 text-center lg:text-left flex-col md:flex-row">
              ...
              <div>
                ...
                <h4 className="font-display font-black text-2xl lg:text-4xl uppercase italic tracking-tighter leading-none mb-2">ABONNEMENT EXPIRÉ</h4>
            ...
            <button 
              ...
              className="w-full lg:w-auto px-10 py-5 bg-white text-red-600 rounded-2xl font-display font-black tracking-[0.2em] text-sm hover:bg-gray-100 transition-all active:scale-95 shadow-2xl flex items-center justify-center gap-3 uppercase italic whitespace-nowrap"
            >
  ...
  const renderSidebarItem = (
    ...
    return (
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all font-medium group active:scale-95 ${isActive ? "bg-brand-600 text-white font-bold shadow-lg shadow-brand-200 dark:shadow-brand-900/20" : isRestrictedByExpiry ? "opacity-40 cursor-not-allowed grayscale" : "text-gray-500 dark:text-gray-400 hover:bg-brand-50 dark:hover:bg-brand-900/20 hover:text-brand-600 dark:hover:text-brand-400"}`}
      >
        <div className="flex items-center space-x-3">
          <div
            className={`transition-transform duration-300 ${isActive ? "scale-110" : isRestrictedByExpiry ? "" : "group-hover:scale-110"}`}
          >
            {icon}
          </div>
        ...
        <div className="flex items-center space-x-2">
          {isLocked && (
            <Lock
              size={12}
              className={isActive ? "text-white/70" : isRestrictedByExpiry ? "text-red-500" : "text-gray-400"}
            />
          )}
          {badge && badge > 0 ? (
            <span
                    ...
                    try {
                      const { error } = await supabase
                        ...
                        .update({
                          ...
                          subscription_status: "active",
                          subscription_end_date: plan.id === "free" ? null : nextMonth.toISOString()
                        })
                        .eq("id", restaurant.id);
                      ...
                      onUpdateRestaurant({
                        ...
                        subscriptionStatus: "active",
                        subscriptionEndDate: plan.id === "free" ? undefined : nextMonth.toISOString(),
                      });
            ...
            <div className="flex flex-col space-y-6">
              ...
              >
                ...
                <button
                  ...
                  disabled={isExpired}
                  className={`w-full bg-white text-gray-900 py-4 rounded-2xl font-display font-bold italic tracking-wide text-xs hover:scale-[1.02] active:scale-95 transition-all shadow-xl relative z-10 ${isExpired ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  Bascule de flux
                </button>
              </div>
              ...
              <div className="space-y-4 pt-4">
                ...
                <button
                  ...
                  className={`w-full bg-brand-600/10 text-brand-600 dark:text-brand-400 font-display font-bold italic tracking-wide text-xs py-4 rounded-2xl hover:bg-brand-600 hover:text-white transition-all border border-brand-500/20 ${isExpired ? 'opacity-50 cursor-not-allowed grayscale' : ''}`}
                >
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="mb-10 text-center">
            <div className="mb-8 flex flex-col items-center">
              ...
              <h4 className="text-3xl font-display font-black text-gray-900 dark:text-white uppercase italic tracking-tighter drop-shadow-sm">
                {restaurant.name}
              </h4>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {[
                            ...
                            <button
                                ...
                                className={`py-3 px-2 rounded-xl text-[10px] font-bold border transition-all ${settingsForm.displayCurrencyMode === mode.id ? 'bg-brand-500 text-white border-brand-600 shadow-lg' : 'bg-white/5 dark:bg-white/5 border-white/10 text-gray-400'}${mode.id === 'cdf' && 'col-span-2 sm:col-span-1'}`}
                            >
                                {mode.label}
                            </button>
                        ))}
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-5xl mx-auto">
        <div className="flex flex-col items-center text-center mb-8">
           ...
           <h2 className="text-3xl font-display font-black text-gray-900 dark:text-white uppercase italic tracking-tight">
             Paramètres du Nexus
           </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 px-4 sm:px-0">
          ...
          >
            ...
            <p className="font-display font-black text-xl text-gray-900 dark:text-white uppercase italic tracking-tight mb-2">
              Vérification
            </p>
          ...
          >
            ...
            <p className="font-display font-black text-xl text-red-600 uppercase italic tracking-tight mb-2">
              Déconnexion
            </p>
  ...
  return (
    ...
    >
      <div className="min-h-[100dvh] bg-gray-50 dark:bg-gray-900 flex transition-colors duration-300">
        ...
        <aside className="hidden lg:flex w-72 flex-col glass border-r border-white/20 dark:border-white/5 h-screen sticky top-0 transition-all duration-500 z-50">
        ...
        <div className="lg:hidden fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-50 px-4 py-3 flex justify-between items-center transition-colors duration-300">
          <div className="flex items-center space-x-2">
        ...
        <main className="flex-1 p-6 md:p-10 pt-20 lg:pt-10 overflow-y-auto h-screen">
                  ...
                  <div className="animate-in fade-in slide-in-from-left-4 duration-300">
                    ...
                    <div className="bg-orange-50 dark:bg-orange-900/20 p-6 rounded-3xl border border-orange-100 dark:border-orange-800">
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                          <p className="text-lg font-black text-gray-900 dark:text-white uppercase italic tracking-tighter leading-none">
